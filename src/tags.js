...
const fetchTags = async (options, remote) => {
  ...
  let tags = (await cmd(`git tag -l --format=${format} ${options.appendGitTag}`)).trim().split("\n").map(parseTag(options)).filter(isValidTag(options));

  if (options.sortTags === "date-desc") {
    tags.reverse();
  } else if (options.sortTags !== "date") {
    tags.sort(sortTags(options));
  }
...
const enrichTag =
  ...
  (t, index, tags) => {
    ...
    return {
      ...
      major: Boolean(previous && semver.valid(t.version) && semver.valid(previous.version) && semver.diff(t.version, previous.version) === "major"),
      minor: Boolean(
        previous && semver.valid(t.version) && semver.valid(previous.version) && ["minor", "preminor"].includes(semver.diff(t.version, previous.version))
      ),
      ...t,
    };
  };
...
const sortTags =
  ...
  ({ version: a }, { version: b }) => {
    console.log(a);
    if (/--sort/.test(options.appendGitTag)) {
      return 0;
    }
